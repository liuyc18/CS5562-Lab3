...
def process_data(data_file_path, seed):
    ...
    random.seed(seed)
    all_data = codecs.open(
        data_file_path, 'r', 'utf-8'
    ).read().strip().split('\n')[1:]
...
def corrupt_gradients(stacked_grads, epsilon):
    ...
    Args:
        stacked_grads (tensor): Individual gradients for each data point, 
            size = (batch_size, 2*768)
    ...
    num_to_corrupt = int(epsilon * stacked_grads.shape[0])
    corrupt_indices = np.random.choice(
        stacked_grads.shape[0], num_to_corrupt, replace=False
    )
    # perturbations = torch.mean(stacked_grads, dim=0) * 1
    corrupt_gradients = stacked_grads.clone()
    corrupt_gradients[corrupt_indices] *= -1
    # print(stacked_grads[corrupt_indices])
    # print(corrupt_gradients[corrupt_indices])
    return corrupt_gradients
...
def corrupt_train_iter(model, batch, labels, optimizer, criterion, epsilon):
    ...
    # Get individual gradients of classifier layer weights, for each data sample
    # in the batch
...
def corrupt_train_epoch(model, tokenizer, train_text_list, train_label_list,
    ...
    for i in tqdm(range(NUM_TRAIN_ITER)):
        batch_sentences = train_text_list[
            i * batch_size: min((i + 1) * batch_size, total_train_len)
        ]
        labels = torch.tensor(
            train_label_list[
                i * batch_size: min((i + 1) * batch_size, total_train_len)
        ...
        loss, acc_num, filtered_cnt = corrupt_train_iter(
            model, batch, labels, optimizer, criterion, epsilon
        )
...
def corrupt_train(train_data_path, model, tokenizer,
    ...
    for epoch in range(epochs):
        ...
        model.train(True)
        train_loss, train_acc, filter_ratio = corrupt_train_epoch(
            model, tokenizer, train_text_list, train_label_list, batch_size, 
            optimizer, criterion, device, epsilon
        )

        logger.info(
            f'\tTrain Loss: {train_loss:.3f}' + 
            f'| Train Acc: {train_acc * 100:.2f}%' + 
            f'| Filter ratio: {filter_ratio * 100:.2f}%'
        )
...
logging.basicConfig(
    filename='corrupt_sgd.log', encoding='utf-8', level=logging.DEBUG
)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--ori_model_path', type=str, help='original model path', 
        default="SST2_clean_model"
    )
    parser.add_argument(
        '--epochs', type=int, help='num of epochs', default=3
    )
    parser.add_argument(
        '--save_model_path', type=str, help='path that new model saved in', 
    ...
    corrupt_train(
        train_data_path, model, tokenizer, BATCH_SIZE, EPOCHS, optimizer, 
        criterion, device, SEED, save_model, save_path, save_metric, EPSILON
    )